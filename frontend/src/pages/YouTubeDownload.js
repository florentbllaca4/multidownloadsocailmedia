// src/components/YouTubeDownload.js
...
const YouTubeDownload = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/download/youtube", { urls: [url] }, { responseType: 'blob' });
      const blob = new Blob([response.data], { type: "application/zip" });
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = "youtube_downloads.zip";
      link.click();
      setLoading(false);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-4 bg-white rounded shadow-lg mt-4">
      <h2 className="text-xl font-semibold mb-4">Shkarkoni nga YouTube</h2>
      <input
        type="text"
        placeholder="Vendosni URL-në e YouTube"
        value={url}
